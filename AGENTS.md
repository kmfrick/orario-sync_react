...
- npm issue: `Exit handler never called` after DNS/network restrictions.
- pip issue: could not resolve/download packages.
- Prevention:
  - For npm in restricted environments, set `npm_config_cache=$PWD/.npm-cache`.
...
- Prevention:
  - Use smoke command: `CI=true npm test -- --watchAll=false --passWithNoTests`.

## Minimum local validation checklist

- Frontend: `npm ci`, `npm run build`, `CI=true npm test -- --watchAll=false --passWithNoTests`.
- Backend:
